...
class CoinbaseAnalyzer:
    ...
    def __init__(self, provider, telegram_writer, target_string, network="testnet", n_previous_blocks=0):
    ...
    async def notify_message(self, block_height, block_hash):
        url = "https://mempool.space/it/testnet/block/" if self.network == "testnet" else "https://mempool.space/it/block/" 
    ...
    async def check_from_previous_n_blocks(self):
        ...
        for block in list_of_blocks[:self.n_previous_blocks]:
            ...
            coinbase_raw_transaction = self.provider.get_raw_coinbase_transaction(block_hash)
            miner_input = self.get_miner_text_input(coinbase_raw_transaction)
            if self.target_string.lower() in miner_input:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--network", "-n", type=str, default="testnet", help="Network (e.g., testnet or mainnet)")
    parser.add_argument("--target", "-t", type=str, default="stratum v2", help="Target string to search in miner input")
    parser.add_argument(
        "--previous", "-p",
        type=int,
        default=0,
    ...
    coinbase_analyzer = CoinbaseAnalyzer(provider, telegram_writer, target_string, network, n_previous_blocks)
